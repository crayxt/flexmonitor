...
class LicEngine {
  // single instance of self shared among all instances
    ...
    switch ($type) {
        case 'FlexLM':
            $file = popen($this->lmutil_loc . " lmcksum -c " . $port . "@" . $server,"r");
            ...
            break;
        case 'LMX':
            $file = popen($this->lmxendutil_loc . " -licstat -host " . $server . " -port " . $port,"r");
            # Let's read in the file line by line
            while (!feof ($file)) {
                $line = fgets ($file, 1024);
                if ( eregi("Feature: .*", $line, $out ) ) {
                    $license = explode(" ", $out[0]);
                    $line = fgets ($file, 1024);
                    $ar=explode(" ",$line);
                    $license[4]=$ar[5];
    ...
    public function get_licmanager_status($server,$port,$type) {
        switch ($type){
            case "LMX":
            $fp = popen($this->lmxendutil_loc . " -licstat -host " . $server . " -port " . $port, "r");
            while ( !feof ($fp) ) {
                $line = fgets ($fp, 1024);
            ...
            case "FlexLM":
                ...
                while ( !feof ($fp) ) {
                    ...
                    if ( eregi (": license server UP", $line) ) {
                        return Array("UP","OK",eregi_replace(".*license server UP .*v", "v", $line));
                        break;
                    }
    ...
    public function get_feature_list($license) {
        ...
        $this->build_license_expiration_array($license[1],$license[2], $license['type'],$expiration_array);
        ...
        foreach ($expiration_array as $key => $feature_array) {
            ...
            $table->addRow(array($key,$total_licenses,$feature_table->toHTML()));
        }
    ...
    switch ($license['type']) {
        ...
        case "LMX":
            # Execute lmutil lmstat -A -c 27000@license or similar
            $fp = popen($this->lmxendutil_loc . " -licstat -host " . $license['hostname'] . " -port " . $license['port'], "r");
            # Feature counter
            $i = -1;
            ...
            while ( !feof ($fp) ) {
                ...
                if (preg_match('/(Feature:) (.*) (v*)/i',$line,$out)){
                    $i++;
                    $license_array[$i]["feature"] = $out[2];
                    $license_array[$i]["licenses_used"] = 0;
                }
                if ( preg_match('/(\d+) (of) (\d+) (.*)/i', $line, $out) && !eregi("No such feature exists", $line) ) {
                    $license_array[$i]["num_licenses"] = $out[3];
                    
                }

                # Count the number of licenses. Each used license has ", start" string in it
                if ( preg_match('/(1 license\(s\) used by) (.*-' . $sitecode . '-.*)/i', $line ) ){
                    $line2 = fgets ($fp, 1024);
                    $users[$i][] = $line . " " . $line2;
                    $license_array[$i]["licenses_used"] += 1;
                }
            }
            foreach ($license_array as $licensetemp) {
